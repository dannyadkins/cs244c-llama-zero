...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Ship the current repo to a remote host and run the Stage 3 GPU-scaling sweep")
    ...
    parser.add_argument("--extra-scaling-args", type=str, default="")
...
def _run_local(cmd: List[str], *, capture_output: bool = True) -> subprocess.CompletedProcess[str]:
...
def _generate_analysis(local_run_dir: Path) -> tuple[Path, Path]:
    figure_path = local_run_dir / "figure3_stage3_scaling.png"
    report_path = local_run_dir / "stage3_scaling_report.md"
    _run_local(
        [
            sys.executable,
            str(PROJECT_ROOT / "analysis" / "stage3_scaling.py"),
            "--run-dir",
            str(local_run_dir),
            "--figure-output",
            str(figure_path),
    ...
    try:
        ...
        _stream_ssh(
            ...
            (
                ...
                f"\"import torch; print('torch', torch.__version__, 'cuda', torch.cuda.device_count())\""
            ),
        )
        ...
        if args.extra_scaling_args.strip():
            remote_cmd.extend(shlex.split(args.extra_scaling_args))
        if args.dry_run:
        ...
        if not args.skip_analysis and not args.dry_run:
            figure_path, report_path = _generate_analysis(local_run_dir)
        ...
        print(f"[remote-stage3-scaling] local results: {local_run_dir}")
    finally:
