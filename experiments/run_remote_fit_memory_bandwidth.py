...
DEFAULT_CONFIG = PROJECT_ROOT / "experiments" / "configs" / "remote_4gpu_small_fit_memory_socket.json"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ship the current repo to a clean remote workspace and run a fit-to-memory bandwidth sweep"
    )
...
def _generate_plots(local_run_dir: Path, plots: Iterable[str]) -> None:
    for plot in plots:
        ...
        try:
            ...
            print(f"[remote-fit-memory] warning: plot {plot} failed: {exc}", file=sys.stderr)
...
def main() -> None:
    ...
    try:
        ...
        remote_cmd = [
            args.remote_python,
            "experiments/run_fit_memory_bandwidth.py",
            "--config",
            str(config_rel),
            "--name",
            run_name,
        ...
        if not args.skip_summary:
            try:
                ...
                print(f"[remote-fit-memory] warning: summary generation failed: {exc}", file=sys.stderr)
        _write_metadata(
            local_run_dir,
            {
                "host": args.host,
        ...
        print(f"[remote-fit-memory] local results: {local_run_dir}")
    finally:
