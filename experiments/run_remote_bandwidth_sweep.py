...
DEFAULT_CONFIG = PROJECT_ROOT / "experiments" / "configs" / "remote_4gpu_bandwidth_smoke.json"
DEFAULT_PLOTS = ["throughput", "comm", "tflops", "stage-throughput", "stage-comm", "loss"]
PLOT_OUTPUTS = {
    "throughput": "throughput.png",
    "comm": "comm.png",
    "tflops": "tflops.png",
    "stage-throughput": "stage_throughput.png",
    "stage-comm": "stage_comm.png",
    "loss": "loss.png",
    "memory": "memory.png",
    "peak-memory": "peak_memory.png",
    "avg-memory": "avg_memory.png",
}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Ship the current repo to a clean remote workspace and run a bandwidth sweep")
    ...
    parser.add_argument("--extra-harness-args", type=str, default="")
    ...
    parser.add_argument("--plots", nargs="+", default=DEFAULT_PLOTS, choices=sorted(PLOT_OUTPUTS))
...
def _run_ssh(args: argparse.Namespace, remote_cmd: str, *, capture_output: bool = True) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
...
def _generate_plots(local_run_dir: Path, plots: Iterable[str]) -> None:
    for plot in plots:
        output_path = local_run_dir / PLOT_OUTPUTS[plot]
        cmd = [
            sys.executable,
        ...
        try:
            ...
            print(f"[remote-sweep] warning: plot {plot} failed: {exc}", file=sys.stderr)


def _generate_report(local_run_dir: Path) -> Path:
    output_path = local_run_dir / "bandwidth_report.md"
    _run_local(
        [
            sys.executable,
            str(PROJECT_ROOT / "analysis" / "bandwidth_report.py"),
            "--run-dir",
            str(local_run_dir),
            "--output",
            str(output_path),
...
def main() -> None:
    ...
    try:
        ...
        if args.install_deps:
            _stream_ssh(
                args,
                f"cd {shlex.quote(remote_workspace)} && {shlex.quote(args.remote_python)} -m pip install -r requirements.txt",
            )
        ...
        if args.extra_harness_args.strip():
            remote_cmd.extend(shlex.split(args.extra_harness_args))
        ...
        if not args.skip_plots:
            _generate_plots(local_run_dir, args.plots)
        if not args.skip_summary:
            try:
                report_path = _generate_report(local_run_dir)
            except subprocess.CalledProcessError as exc:
                print(f"[remote-sweep] warning: summary generation failed: {exc}", file=sys.stderr)
        _write_metadata(
            local_run_dir,
            {
                "host": args.host,
        ...
        print(f"[remote-sweep] local results: {local_run_dir}")
    finally:
